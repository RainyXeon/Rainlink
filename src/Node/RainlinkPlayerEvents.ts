...
export class RainlinkPlayerEvents {
	...
	protected TrackStartEvent(manager: Rainlink, data: Record<string, any>) {
		...
		if (player) {
			...
			manager.emit(
				...
				'[Rainlink] -> [Player] -> [Events] -> [Start] | ' + JSON.stringify(data),
			);
		}
		return;
	}
	...
	protected TrackExceptionEvent(manager: Rainlink, data: Record<string, any>) {
		...
		if (player) {
			...
			manager.emit(
				RainlinkEvents.Debug,
				'[Rainlink] -> [Player] -> [Events] -> [Exception] | ' + JSON.stringify(data),
			);
		}
		return;
	}
	...
	protected TrackStuckEvent(manager: Rainlink, data: Record<string, any>) {
		...
		if (player) {
			...
			manager.emit(
				RainlinkEvents.Debug,
				'[Rainlink] -> [Player] -> [Events] -> [Stuck] | ' + JSON.stringify(data),
			);
		}
		return;
	}
	...
	protected PlayerUpdate(manager: Rainlink, data: Record<string, any>) {
		...
		if (player) {
			...
			manager.emit(
				RainlinkEvents.Debug,
				'[Rainlink] -> [Player] -> [Events] -> [Updated] | ' + JSON.stringify(data),
			);
